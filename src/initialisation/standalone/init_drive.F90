...
IF ( z1_tq_vary ) THEN
  IF ( LEN_TRIM(z1_tq_file) == 0 )                                            &
      CALL log_fatal("init_drive", "No file name provided for z1_tq_file.")
...
END IF
!-----------------------------------------------------------------------------
! check precip perturbation factor is greater than zero
IF ( l_perturb_driving ) THEN
  IF ( precip_rel_perturbation < 0.0 )                                        &
    CALL log_fatal("init_drive",                                              &
...
ELSE IF ( ANY(var(1:nvars) == 'lw_net') .AND.                                 &
  ...
  ! If net downward longwave and shortwave radiations are given, we need to use
  !  io_rad_type = 3
  io_rad_type = 3
...
ELSE
  ...
  ! We raise a fatal error if none of the appropriate forcing pairs are present.
 
  CALL log_fatal ("init_drive", "Not enough radiation variables or "    //    &
...
IF (nvars_radiation > 2) THEN
  CALL log_fatal ("init_drive", "Too many radiation variables " //            &
