...
! Array arguments with intent(out).
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) :: leached(land_pts)
...
IF ( l_top ) THEN
  DO iz = 1,nz_soilc
    leached_lat(:,iz) = solute_flux_h( land_pts, vs_pts, dz_soilc(iz),        &
                                       vs_index, conc(:,iz),                  &
...
! Local arrays.
!-----------------------------------------------------------------------------
REAL :: leached_lay(land_pts,nz_soilc)
...
FUNCTION solute_flux_h( land_pts, vs_pts, dz, vs_index, conc,                 &
                        water_flux_lateral)                                   &
         RESULT(solflux)
...
! Function result.
!-----------------------------------------------------------------------------
REAL :: solflux(land_pts) ! Solute flux between layers (kg m-2 s-1).
