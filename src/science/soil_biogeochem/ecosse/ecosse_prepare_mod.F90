...
SUBROUTINE ecosse_prepare( land_pts, s, triffid_call, vs_pts,                 &
     vs_index, frac_surft_start, nlayer_nitrif, convert_veg_input, dt_input,  &
...
LOGICAL ::                                                                    &
  l_calculate_inputs,                                                         &
    !  Switch indicating when plant inputs are to be calculated.
  l_interp
...
IF ( triffid_call == 0) THEN
  ...
  l_calculate_inputs = .TRUE.
  l_add_plant_inputs = .TRUE.
  ! Get input timestep length.
  dt_input = FLOAT( triffid_period * isec_per_day )
  ! Get timestep for plant inputs as a fraction of veg model timestep.
  convert_veg_input = 1.0
...
USE soil_ecosse_vars_mod, ONLY:                                               &
  ! imported arrays
  plant_input_c_gb, plant_input_n_gb
...
!-----------------------------------------------------------------------------
CALL profile_plant_inputs( land_pts, f_root_pft, input_frac )
...
! rates.
!-----------------------------------------------------------------------------
plant_input_c_gb(:) = SUM( plant_input_c_dpm + plant_input_c_rpm, 2 )         &
                      / dt_input

IF ( l_soil_N ) THEN
  plant_input_n_gb(:) = SUM( plant_input_n_dpm + plant_input_n_rpm, 2 )       &
                        / dt_input
END IF
...
SUBROUTINE profile_plant_inputs( land_pts, f_root_pft, input_frac )
...
USE jules_soil_ecosse_mod, ONLY:                                              &
  ! imported scalar parameters
  plant_input_exp, plant_input_roots,                                         &
  ! imported scalars
  plant_input_profile,                                                        &
  ! imported arrays
  dz_soilc
...
! Array arguments with intent(in)
!-----------------------------------------------------------------------------
REAL, INTENT(IN) :: f_root_pft(npft,nz_soilc)
    ! Fraction of roots in each soil layer.

!-----------------------------------------------------------------------------
! Array arguments with intent(out).
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) :: input_frac(npft,nz_soilc)
    ! The proportion of plant inputs going into each soil layer.
...
REAL :: dzSum       ! Accumulated depth (m).
REAL :: piRemains   ! Remaining fraction of inputs.
...
REAL, INTENT(IN) :: clay(land_pts,nz_soilc)
...
REAL, INTENT(IN) :: soil_pH(land_pts,nz_soilc)
    ! pH of each layer.

!-----------------------------------------------------------------------------
! Array arguments with intent(out)
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) :: biohum_nc(land_pts,nz_soilc)
