...
USE trif_vars_mod, ONLY:                                                      &
  ! imported arrays
  ! Note that many of these have a local equivalent (e.g. minl_n) which
  ! differs only in the units used. The local versions are used to work in
  ! SI units, the imported version to provide a diagnostic in different units.
  minl_n_gb, minl_n_pot_gb, immob_n_gb, immob_n_pot_gb, fn=>fn_gb,            &
  resp_s_diag_gb, resp_s_pot_diag_gb
...
immob_n_gb(:,:,:)         = immob_n_gb(:,:,:)    * dt_to_360d
...
! Array arguments with intent(out)
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) :: rate_factor(land_pts,nlayer)
    ! Overall (combined) rate modification factor.
