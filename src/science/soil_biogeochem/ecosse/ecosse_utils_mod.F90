...
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) ::  outval(np,nzOut)
   ! Output field.
...
! Array arguments with intent(out)
!-----------------------------------------------------------------------------
REAL, INTENT(OUT) :: residual_n(land_pts,nz_soilc)
...
REAL :: dpool  ! Amount added to a pool (kg m-2).
...
DO i = 1,vs_pts
  ...
  ! minimum-allowed value. No consideration is taken of the C:N ratio - that is
  ! dealt with separately.
  ! A more complicated alternative would be to consider if other pools can
  ! donate C.
  ! Any C added is removed from the 
  !---------------------------------------------------------------------------
  ! C DPM.
  !---------------------------------------------------------------------------
  DO iz = 1,nz_soilc
