...
MODULE tile_albedo_mod
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='TILE_ALBEDO_MOD'
CONTAINS
...
USE jules_surface_types_mod,  ONLY:                                           &
  npft, ntype, lake, soil, ice, urban_canyon

USE water_constants_mod,      ONLY:                                           &
  rho_ice, tm

USE jules_snow_mod,           ONLY:                                           &
  kland, maskd, tcland, rho_snow_const, cansnowtile, l_snowdep_surf
...
USE lake_mod,                 ONLY:                                           &
  albedo_whiteice_ref, albedo_blueice_ref, c_albice_MR, lake_h_ice_gb

USE ancil_info,               ONLY:                                           &
l_soil_point, l_lice_point, nsoilt, l_lice_surft

USE jules_mod,                ONLY:                                           &
  snowdep_surft, albobs_scaling_surft

USE prognostics,              ONLY:                                           &
  snowdepth_surft, rho_snow_grnd_surft, nsnow_surft, sice_surft, sliq_surft,  &
  ds_surft
    ...
    DO band = 1,2
      DO n = npft+1,ntype
        ...
        DO j = 1,surft_pts(n)
          l = surft_index(j,n)
          IF ( n == soil ) THEN
            alb_type(l,n,2 * band-1) = MIN(MAX(albsoil_soilt(l,m) *           &
                                             albsfsc(l,band),                 &
                                             albsnf_nvgl(n - npft)),          &
                                         albsnf_nvgu(n - npft))

            albobs_scaling_surft(l,n,band) = alb_type(l,n,2 * band-1)         &
                                              / albsoil_soilt(l,m)
