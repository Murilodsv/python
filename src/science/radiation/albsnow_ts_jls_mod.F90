...
USE science_fixes_mod, ONLY: l_fix_albsnow_ts
...
REAL, INTENT(IN) :: albudir(land_pts,2)
                             ! Direct albedos of the substrate
REAL, INTENT(IN) :: albudif(land_pts,2)
                             ! Diffuse albedos of the substrate
REAL, INTENT(IN) :: cosz_gb(land_pts)
                             ! Cosine of the zenith angle.
REAL, INTENT(IN) :: rgrain(land_pts)
                             ! Snow grain size (microns).
REAL, INTENT(IN) :: snowmass(land_pts)
                             ! Mass loading of snow
REAL, INTENT(IN) :: soot_gb(land_pts)
                              ! Snow soot content (kg/kg).

!   Array arguments with intent(out):
REAL, INTENT(OUT) :: alb_snow(land_pts,4)
!                              Snow albedo.
