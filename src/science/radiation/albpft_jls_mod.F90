...
REAL :: exp_hla, exp_minus_hla, exp_minus_kla

!Optimisation variables to reduce the number of IF evaluations
REAL :: omega_arr(2),  alpa_arr(2),                                           &
...
!$OMP             PRIVATE(albdif_tmp, albscl_new, albscl_tmp, b,           &
!$OMP                     betadif, betadir, c, ca, coszm, d, d1, d2,       &
!$OMP                     dabeer_dla, dlai, drdird_dlai, drdiru_dlai,      &
!$OMP                     drdifu_dlai, drdifd_dlai, f, g, h, h1, h2, h3,   &
!$OMP                     h4, h5, h6, h7, h8, h9, h10, i, k, l, la, p1,    &
!$OMP                     p2, p3, p4, rdifd, rdifu, rdird, rdiru, rnet_dif,&
!$OMP                     rnet_dir, s1, s2, salb, sig, sqcost, taul, u1,   &
!$OMP                     u2, u3, exp_hla, exp_minus_hla, exp_minus_kla,   &
!$OMP                     om, alpl)                                        &
!$OMP SHARED(surft_pts, surft_index, albpft_call, band,                    &
!$OMP        albobs_scaling_surft,  n, orient, l_getprofile,               &
...
!$OMP        omega_arr, alpa_arr, omegal_arr, alpal_arr, omegau_arr,       &
!$OMP        alpau_arr)                                                    &
!$OMP SCHEDULE(STATIC)
    DO j = 1,surft_pts(n)
      l = surft_index(j,n)
