...
REAL, PARAMETER :: s_df_fit(1:4) = (/-0.1482,-0.012,0.1609,-0.0244 /)
...
REAL, PARAMETER :: wl_min = 2.0e-8 ! 200nm
REAL, PARAMETER :: wl_max = 5.0e-6 ! 5000nm
...
REAL, INTENT(OUT) :: cosz(nd_points)
REAL, INTENT(OUT) :: ws_10m(nd_points)
REAL, INTENT(OUT) :: chloro(nd_points)
...
! x of input
REAL, INTENT(IN) :: x_in(nd_in)
! y of input
REAL, INTENT(IN) :: y_in(nd_in)
! x of output
REAL, INTENT(IN) :: x_out(nd_out)

! output values of y, at x_out
REAL, INTENT(OUT) :: y_out(nd_out)
...
INTEGER, INTENT(IN) :: nd_points
! cosine of the solar zenith angle:
REAL, INTENT(IN) :: cosz(nd_cosz)
! sigma (wind speed param from Cox and Munk)
REAL, INTENT(IN) :: sigma(nd_points)

! output:
REAL, INTENT(OUT) :: zmod(nd_points)

! local vars:
REAL, PARAMETER :: p(1:11) = (/0.0152, -1.7873, 6.8972, -8.5778, 4.071,       &
...
REAL, INTENT(IN) :: wl(nd_wl)

! output:
REAL, INTENT(OUT) :: refm(nd_wl)
...
REAL, INTENT(IN) :: cosz(nd_cosz)
! chlorophyll content
REAL, INTENT(IN) :: chloro(nd_points)

! output:
REAL, INTENT(OUT) :: r0(nd_points)

! local vars:
REAL, PARAMETER :: aw440 = 0.00635
REAL :: chlabs(1)
REAL :: aw(1)
REAL :: bw(1)
REAL :: ylmd
REAL :: ap(nd_points)
REAL :: bp550(nd_points)
REAL :: chl_v(nd_points)
REAL :: bbp(nd_points)
REAL :: hb(nd_points)
REAL :: f(nd_points)
! parameters in this param are for wavelengths in nm:
REAL :: wl_nm(1)
! and chlorophyll in g m-3:
REAL :: chloro_gm3(nd_points)

! lookup table for chlabs (achl and wl_achl):
INTEGER, PARAMETER :: n_chlabs_lu = 38
REAL :: chlabs_lu(n_chlabs_lu)
REAL :: wl_chlabs(n_chlabs_lu)
...
DO ipt = 1, nd_points
  chloro_gm3(ipt) = chloro(ipt) * 1.0e3
END DO
...
DO ipt = 1, nd_points
  ap(ipt) = (0.06 * chlabs(1) * (chloro_gm3(ipt)**0.65)) +                    &
              0.2 * (aw440+0.06 * (chloro_gm3(ipt)**0.65)) * ylmd
END DO

DO ipt = 1, nd_points
  bp550(ipt) =  0.416 * chloro_gm3(ipt)**0.766
END DO

DO ipt = 1, nd_points
  IF (chloro_gm3(ipt) > 2.0) THEN
    chl_v(ipt) = 0.0
  ELSE
    chl_v(ipt) = 0.5 * ( LOG10(chloro_gm3(ipt)) - 0.3 )
  END IF
END DO

DO ipt = 1, nd_points
  IF (chloro_gm3(ipt) > 0.02) THEN
    bbp(ipt) = (0.002+0.01 * (0.5-0.25 * LOG10(chloro_gm3(ipt))) *            &
