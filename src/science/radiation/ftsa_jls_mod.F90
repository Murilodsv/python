...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='FTSA_MOD'
...
USE science_fixes_mod, ONLY: l_fix_alb_ice_thick

USE water_constants_mod, ONLY: tm, tfs
USE fluxes, ONLY: alb_sicat
...
REAL :: pond_frac_cat_use    ! Meltpond fraction on sea ice categories
                             ! to be used in calculation.
                             ! (set to zero if pond depth < 0.004 m, or
                             ! to pond_frac_cat otherwise).

REAL, ALLOCATABLE :: cosz_s(:)
REAL, ALLOCATABLE :: ws_10m_s(:)
REAL, ALLOCATABLE :: chloro_s(:)
REAL, ALLOCATABLE :: saos_s(:,:,:)
...
REAL, PARAMETER :: chloro_constant = 5.0e-7
...
REAL :: ice_alb     ! Sea ice albedo

REAL :: hice(n_points,nice_use)
!      Ice thickness

REAL :: saos_bb(n_points,2)
!      Broadband open sea albedo, for methods that need it.
REAL :: spectralsea(n_bands)
!      Spectral albedo scaling factor, for methods that need it.
REAL, PARAMETER :: spec_band_bb(1) = -1.0
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='FTSA'
...
ELSE
  ...
  CALL ereport ('ftsa',errcode,errmsg)
END IF
      ...
      DO l = 1, sice_pts_ncat(n)
        ...
        IF (l_sice_scattering) THEN
          ...
          ELSE
            errcode = 555
            errmsg =                                                          &
         "Semtner zero-layer approximation should not be used with CICE multilayers."
...
ELSE
  ...
  ! Consistency check
  IF (l_ssice_albedo .AND. l_sice_scattering .AND. l_sice_swpen) THEN
    errcode= 555
    errmsg =                                                                  &
    "Semtner zero-layer approximation should not be used with CICE multilayers."
        ...
        ELSE           ! no snow so bare ice only
          ...
          IF (l_sice_meltponds) THEN
            ...
            IF (tstar_sice_cat(j,n) > (tm - dt_bare)) THEN
              alb_sicat(ll,n,1) = alphab + (tstar_sice_cat(j,n) - tm + dt_bare) &
                                 *dalb_bare_wet
            ELSE      ! surface is dry
              alb_sicat(ll,n,1) = alphab
          ...
          IF (l_sice_scattering) THEN
            ! Semtner modification dry albedo for internal
            ! scattering (Semtner, 1976)
            ! Note that this can only be applied when l_sice_swpen = .false.
