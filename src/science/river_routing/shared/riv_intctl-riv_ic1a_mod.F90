...
#if !defined(UM_JULES)
USE model_grid_mod, ONLY: global_land_mask
#endif
IMPLICIT NONE
...
END DO
#else
sub_runoffin(:,:) = UNPACK(tot_sub_runoff, global_land_mask,                  &
                           sub_runoffin)
surf_runoffin(:,:) = UNPACK(tot_surf_runoff,                                  &
                            global_land_mask, surf_runoffin)
smvcst_g(:,:) = UNPACK(smvcst, global_land_mask, smvcst_g)
smvcwt_g(:,:) = UNPACK(smvcwt, global_land_mask, smvcwt_g)
smcl_dsm_g(:,:) = UNPACK(smcl, global_land_mask, smcl_dsm_g)
sthu_dsm_g(:,:) = UNPACK(sthu, global_land_mask, sthu_dsm_g)
#endif
