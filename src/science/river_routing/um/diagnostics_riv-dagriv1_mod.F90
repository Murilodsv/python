...
! --------------------------------------------------------------------
! River outflow - ATMOS grid (at seapoints)
! --------------------------------------------------------------------
...
! --------------------------------------------------------------------
! River outflow - TRIP grid (at seapoints)
! --------------------------------------------------------------------
! Item 26 005 riverout_rgrid
IF (sf(005,sect)) THEN
  ! DEPENDS ON: copydiag
  CALL copydiag(STASHwork(si(005,sect,im_index)),riverout_rgrid,              &
       river_row_length,river_rows,0,0,0,0, at_extremity_riv,                 &
       atmos_im,sect,005,                                                     &
       icode,cmessage)

  IF (icode  >   0) THEN
    cmessage="Error in copydiag( item 005)"
    GO TO 9999
  END IF
END IF
