...
REAL, PARAMETER :: sthu_min = 0.01 ! required by psi_from_sthu
...
REAL, INTENT(IN) :: v(npnts), extra_factor(npnts)  
                            ! v*extra_factor is the volumetric soil moisture
!                             !    concentration in layer
!                             !    (m3 H2O/m3 soil).  
REAL, INTENT(IN) :: v_close(npnts)  ! Volumetric soil moisture
!                             !    concentration below which
!                             !    stomata close (m3 H2O/m3 soil).
REAL, INTENT(IN) :: v_open(npnts)  ! Volumetric soil moisture
!                             !    concentration above which
!                             !    stomatal aperture is not limited by soil water.
!                             !    (m3 H2O/m3 soil).
REAL, INTENT(IN) :: psi(npnts)
!                             ! (negative) soil water potential (in Pa)

! internal
REAL :: sthu, x, x_open, x_close

! returns
REAL :: fsmc_l(npnts)  ! soil water availability factor for this soil layer
