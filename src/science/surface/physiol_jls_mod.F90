...
SUBROUTINE physiol (                                                          &
  ...
  wt_ext_soilt,albsoil_soilt,cos_zenith_angle,                                &
...
USE ancil_info, ONLY: l_soil_point, dim_cslayer, nsoilt
...
USE jules_surface_mod, ONLY: diff_frac, l_aggregate

USE sf_diags_mod, ONLY: strnewsfdiag

USE jules_vegetation_mod, ONLY: l_crop, l_irrig_dmd, l_use_pft_psi, l_triffid
...
USE trif_vars_mod, ONLY: fapar_diag_pft, gpp_gb_out, gpp_pft_out,             &
                         gpp_gb_acc, gpp_pft_acc
...
USE crop_vars_mod, ONLY:                                                      &
  rootc_cpft, gs_irr_surft, smc_irr_soilt,                                    &
  sthu_irr_soilt, frac_irr_soilt, frac_irr_surft, wt_ext_irr_surft,           &
  gc_irr_surft

USE p_s_parms, ONLY: bexp_soilt, sathh_soilt,                                 &
                      v_close_pft, v_open_pft
...
,gsoil_irr_soilt(land_pts,nsoilt)         ! WORK Bare soil conductance over
!                                 !      irrigated fraction.
...
REAL :: lai_pft_soil_evap(land_pts,npft)
...
!dim_cslayer is a parameter in UM_JULES and a variable in JULES
...
!$OMP resp_l_pft,resp_r_pft,fsmc_pft,isoprene_pft,terpene_pft,              &
...
!$OMP dim_cs1,resp_s_soilt,gpp,npp,resp_p,ra,canhc,vfrac,isoprene_gb,       &
!$OMP terpene_gb,methanol_gb,acetone_gb,fsoil_tot,frac,                     &
...
!$OMP gsoil_irr_soilt, smvccl_soilt, gs_nvg, soil,                          &
...
!$OMP DO SCHEDULE(STATIC)
  DO l = 1,land_pts
    gsoil_soilt(l,m) = 0.0
    IF (smvccl_soilt(l,m,1) > 0.0) THEN
...
!$OMP DO SCHEDULE(STATIC)
    DO l = 1,land_pts
      ...
      IF (smvccl_soilt(l,m,1) > 0.0) THEN
        gsoil_irr_soilt(l,m) = gs_nvg(soil - npft) * (sthu_irr_soilt(l,m,1)   &
                                * smvcst_soilt(l,m,1) / smvccl_soilt(l,m,1))**2
      END IF
    END DO
!$OMP END DO NOWAIT
  ...
  IF ( albpft_call > 0 ) THEN
    ...
    DO n = 1,npft
      ...
      DO l = 1,land_pts
        ...
        !Note that albobs scaling is not soil tile-aware, so may behave
        !unxepectedly when used with soil tiling.
        albudif(l,1,n) = albsoil_soilt(l,m) * albobs_scaling_surft(l,soil,1)
        albudif(l,2,n) = albsoil_soilt(l,m) * albobs_scaling_surft(l,soil,2)
        albudir(l,1,n) = albsoil_soilt(l,m) * albobs_scaling_surft(l,soil,1)
        albudir(l,2,n) = albsoil_soilt(l,m) * albobs_scaling_surft(l,soil,2)
      END DO
    END DO
  ELSE
    DO n = 1,npft
      ...
      DO l = 1,land_pts
        albudif(l,1,n) = albsoil_soilt(l,m)
        albudif(l,2,n) = albsoil_soilt(l,m)
        albudir(l,1,n) = albsoil_soilt(l,m)
        albudir(l,2,n) = albsoil_soilt(l,m)
      END DO
    END DO
  END IF
...
!$OMP END PARALLEL DO
  ...
  CALL soil_evap (land_pts,sm_levels,surft_pts(n),surft_index(:,n),           &
                  gsoil_soilt(:,m),lai_pft_soil_evap(:,n),gs_type(:,n),       &
                  wt_ext_type(:,:,n),fsoil(:,n),gsoil_irr_soilt(:,m),         &
...
!$OMP END PARALLEL DO
  ...
  DO m = 1, nsoilt
    CALL microbe (land_pts,dim_cs1,dim_cs2,l_q10,cs_pool_soilt(:,m,:,:),      &
                  ...
                  resp_s_soilt(:,m,:,:),veg_frac,sf_diag)
  END DO
END SELECT
...
!$OMP             SCHEDULE(STATIC)
    DO j = 1,surft_pts(n)
      ...
      DO k = 1,sm_levels
        wt_ext_soilt(l,m,k) = wt_ext_soilt(l,m,k) + frac(l,n) * wt_ext_type(l,k,n)
        IF (l_irrig_dmd) THEN 
          ...
          IF ( frac_irr_soilt(l,m) > EPSILON(1.0) ) THEN
            smc_irr_soilt(l,m) = smc_irr_soilt(l,m)                           &
                           + MAX(0.0,                                         &
                                 ...
                                 * frac(l,n) * wt_ext_irr_type(l,k,n)         &
                                 * rho_water * dzsoil(k)                      &
                                 * (sthu_irr_soilt(l,m,k) * smvcst_soilt(l,m,k) &
                                    - v_close_pft(l,k,n)))
          END IF
        END DO
!$OMP END PARALLEL DO
