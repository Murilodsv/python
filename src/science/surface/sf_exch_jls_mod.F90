...
USE science_fixes_mod, ONLY: l_fix_ctile_orog, l_fix_wind_snow
...
,cdr10m_n(pdims_s%i_start:pdims_s%i_end,pdims_s%j_start:pdims_s%j_end)        &
                       ! OUT Phi_neutral(10m)/Phi(Level_1)
                       !     Pho on level 1 usually includes orography
,cd10m_n(pdims_s%i_start:pdims_s%i_end,pdims_s%j_start:pdims_s%j_end)         &
                       ! OUT Neutral 10 m drag coefficient
...
!$OMP SHARED(land_pts,nsurft,z0_urban,ftl_surft,rib_surft,z0m_surft,         &
!$OMP u_s_std_surft,chr1p5m,resfs,snowdep_surft,snowdepth,sf_diag)           &
...
!$OMP                 ztm_gb, z0h_z0m, rib_surft, db_surft, z0h_surft,       &
...
!$OMP END PARALLEL DO
  ...
  CALL sf_resist (                                                            &
   ...
   resft(:,n),gc_irr_surft(:,n),resfs_irr_surft(:,n),sf_diag%resfs_stom(:,n), &
...
! Land tiles
!$OMP PARALLEL DO IF(nsurft > 1) DEFAULT(NONE) PRIVATE(n)                     &
!$OMP SHARED(nsurft, land_pts, surft_pts, land_index, surft_index, bq_1,      &
!$OMP        bt_1, qstar_surft, q_elev, resft, t_elev, tstar_surft, vshr_land,&
!$OMP        z0h_surft, z0m_surft, z1_tq, z1_uv, rib_surft, db_surft,         &
!$OMP        zdt_surft, l_vegdrag_surft)     SCHEDULE(STATIC)
DO n = 1,nsurft
  CALL sf_rib (                                                               &
   land_pts,surft_pts(n),land_index,surft_index(:,n),                         &
...
IF (formdrag ==  effective_z0) THEN
!$OMP PARALLEL DO IF(nsurft > 1) DEFAULT(NONE) PRIVATE(n)                    &
!$OMP SHARED(nsurft, land_pts, surft_pts, land_index, surft_index,           &
!$OMP        fd_stab_dep, orog_drag_param, ho2r2_orog, rib_surft, sil_orog,  &
!$OMP        z0m_surft, z1_uv, wind_profile_factor, z0m_eff_surft )          &
!$OMP             SCHEDULE(STATIC)
  DO n = 1,nsurft
    CALL sf_orog (                                                            &
     land_pts,surft_pts(n),land_index,surft_index(:,n),                       &
...
!$OMP END PARALLEL DO
  CALL sf_resist (                                                            &
   ...
   gc(:,n),gc_stom_surft(:,n),snowdep_surft(:,n),vshr_land,fraca(:,n),        &
   resfs(:,n),resft(:,n),                                                     &
   gc_irr_surft(:,n),resfs_irr_surft(:,n),sf_diag%resfs_stom(:,n),            &
   sf_diag%l_et_stom,sf_diag%l_et_stom_surft)   
...
IF ( .NOT. l_aggregate .AND. can_model == 4) THEN
  DO n = 1,npft
    IF ( cansnowtile(n) ) THEN
!$OMP PARALLEL DO IF(surft_pts(n) > 1) DEFAULT(NONE) PRIVATE(i, j, k, l)       &
!$OMP             SHARED(ch_surft, fraca, gc, land_index, l_irrig_dmd, n,      &
...
!$OMP                    surft_index, t_i_length, vshr_land) SCHEDULE(STATIC)
      DO k = 1,surft_pts(n)
        ...
        IF (snow_surft(l,n) >  0.0) THEN
          fraca(l,n) = 0.0
          resfs(l,n) = gc(l,n) /                                              &
                  (gc(l,n) + ch_surft(l,n) * vshr_land(i,j))
          resft(l,n) = resfs(l,n)
          IF (l_irrig_dmd) THEN
            resfs_irr_surft(l,n) = resfs(l,n)
          END IF
...
!$OMP END PARALLEL DO
  ...
  CALL sf_flux (                                                              &
   ...
   nsnow(:,n),n,canhc_surf(:),dzsurf,hcons_surf,                              &
   qs1_elev(:,n),qstar_surft(:,n),q_elev(:,n),                                &
   radnet_surft(:,n),resft(:,n),rhokh_1(:,n),l_soil_point,                    &
   ...
   1.0,anthrop_heat(:,n),scaling_urban(:,n),l_vegdrag_surft(n),               &
   fqw_1,ftl_1,                                                               &
   alpha1(:,n),ashtf_prime_surft(:,n),fqw_surft(:,n),                         &
   epot_surft(:,n),ftl_surft(:,n),                                            &
   dtstar_surft(:,n),sea_point                                                &
 )
END DO
...
!$OMP SHARED(pdims,cdr10m_n,cd10m_n)
...
IF (sf_diag%su10 .OR. sf_diag%sv10 .OR. sf_diag%sq1p5 .OR.                    &
  ...
  CALL sfl_int (                                                              &
     ...
     sf_diag,                                                                 &
     cdr10m,cdr10m_n,cd10m_n,chr1p5m_sea,chr10m_sea                           &
     )
  !
  ...
  DO n = 1,nice_use
    CALL sfl_int (                                                            &
       ...
       sf_diag,                                                               &
       cdr10m,cdr10m_n,cd10m_n,chr1p5m_sice(:,:,n),                           &
       chr10m_sice(:,:,n)                                                     &
       )
  END DO
  !
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(j,i)                                                            &
!$OMP SHARED(tdims,z0m_gb,fz0,z0h_gb,fz0h,z0m_eff,z0h_eff)
DO j = tdims%j_start,tdims%j_end
  DO i = tdims%i_start,tdims%i_end
    z0m_gb(i,j) = lb * EXP( - SQRT(1.0 / fz0(i,j)) )
...
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i, j, l) SHARED(land_pts, land_index,  &
!$OMP             t_i_length, z0h_gb, z0h_gb_land, z0m_gb, z0m_gb_land)        &
!$OMP             SCHEDULE(STATIC)
DO l = 1,land_pts
  j=(land_index(l) - 1) / t_i_length + 1
  i = land_index(l) - (j-1) * t_i_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(j,i)                                                            &
!$OMP SHARED(tdims,rhokm_1,flandg,rhokm_land,rhokm_ssi)
DO j = tdims%j_start,tdims%j_end
  DO i = tdims%i_start,tdims%i_end
