...
USE crop_vars_mod, ONLY: frac_irr_soilt, frac_irr_surft, wt_ext_irr_surft     &
                         ,ext_irr_soilt, smc_irr_soilt, resfs_irr_surft
...
!                            !     (kg per sq m per s).
,ext_soilt(land_pts,nsoilt,nshyd)
...
!$OMP DO SCHEDULE(STATIC)
    DO k = 1,surft_pts(n)
      ...
      IF (l_irrig_dmd) THEN
        ! irrigated fraction
        IF ( frac_irr_soilt(l,mm) > EPSILON(1.0) ) THEN
          ext_irr_soilt(l,mm,m) = ext_irr_soilt(l,mm,m) + tile_frac(l,n) *    &
                                  wt_ext_irr_surft(l,m,n)                     &
                                  * esoil_irr_surft(l,n) *                    &
                                  frac_irr_surft(l,n) / frac_irr_soilt(l,mm)
          ! hadrd - with original code, FRAC_IRR_surft(L,N)/FRAC_IRR(L) 
