...
! Arguments
REAL, INTENT(IN) :: flandg(tdims%i_start:tdims%i_end,                         &
                           tdims%j_start:tdims%j_end)
! Land fractions in each grid box
REAL, INTENT(IN) :: ice_fract(tdims%i_start:tdims%i_end,                      &
                              tdims%j_start:tdims%j_end)
! Ice fractions (this scheme will not be called with multiple categories)
REAL, INTENT(IN) :: z1_tq_sea(tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end)
! Height of lowest model level relative to sea
REAL, INTENT(IN) :: z1_tq_top_sea(tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end)
! Height of top of lowest model level relative to sea
REAL, INTENT(IN) :: z0m_ice(tdims%i_start:tdims%i_end,                        &
                            tdims%j_start:tdims%j_end)
! Momemtum roughness length over ice 
REAL, INTENT(IN) :: z0m_sea(tdims%i_start:tdims%i_end,                        &
                            tdims%j_start:tdims%j_end)
! Momemtum roughness length over open water
REAL, INTENT(IN) :: cd_int_stb(tdims%i_start:tdims%i_end,                     &
                               tdims%j_start:tdims%j_end)
! Interfacial drag coefficient over pack ice (including stability)
REAL, INTENT(IN) :: cd_sea_stb(tdims%i_start:tdims%i_end,                     &
                               tdims%j_start:tdims%j_end)
! Drag coefficient over open ocean (including stability)

REAL, INTENT(OUT) :: cd_frm(tdims%i_start:tdims%i_end,                        &
                            tdims%j_start:tdims%j_end)
