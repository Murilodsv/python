...
SUBROUTINE sf_flux (                                                          &
 ...
 fqw_1_gb,ftl_1_gb,                                                           &
 alpha1,ashtf_prime,fqw_1,epot,ftl_1,dtstar,                                  &
 sea_point                                                                    &
 )
...
USE planet_constants_mod, ONLY: cp, g, r, c_virtual,epsil=>repsilon
...
! Note: This modification only applies when vf_nvg = 0.

!$OMP DO SCHEDULE(STATIC)
DO k = 1,surft_pts
  l = surft_index(k)
  IF ( .NOT. l_aggregate     .AND. l_moruses_storage                          &
       .AND. l_soil_point(l) .AND. n == urban_roof ) THEN
    hcons(l) = 0.0
  END IF
