...
REAL, INTENT(IN) :: diam                  !IN particle diameter
REAL, INTENT(IN) :: rhop                  !IN particles density
REAL, INTENT(IN) :: p(tdims%i_start:tdims%i_end,                              &
...
REAL, INTENT(OUT) :: vstokes( tdims%i_start:tdims%i_end,                      &
                              tdims%j_start:tdims%j_end,                      &
                              nlevs)
                     !OUT sedimentation velocity

REAL, INTENT(OUT) :: etaa(    tdims%i_start:tdims%i_end,                      &
                              tdims%j_start:tdims%j_end,                      &
                              nlevs)
                     !OUT viscosity of air

REAL, INTENT(OUT) :: ccf(     tdims%i_start:tdims%i_end,                      &
                              tdims%j_start:tdims%j_end,                      &
                              nlevs)
...
REAL :: tc(       tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end)
  !LOC temperature in deg C
REAL :: lamdaa(   tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end)
  !LOC mean free path of particle
REAL :: alphaccf( tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end)
  !LOC
