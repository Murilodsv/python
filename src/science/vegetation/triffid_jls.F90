...
USE pftparm, ONLY: a_wl, a_ws, b_wl, eta_sl, lma, rootd_ft, sigl
...
USE prognostics, ONLY: wood_prod_fast_gb, wood_prod_med_gb,                   &
                       ...
                       t_soil_soilt_acc, n_inorg_avail_pft, ns_pool_gb,       &
                       triffid_co2_gb
...
USE trif_vars_mod, ONLY: cnsrv_veg_triffid_gb, cnsrv_soil_triffid_gb,         &
                         ...
                         n_fertiliser_gb, root_abandon_n_pft, npp_n_gb, npp_n,&
...
REAL, PARAMETER :: fire_ratio = 0.13
...
INTEGER ::                                                                    &
  zlyr,                                                                       &
    ! Soil level to which fertiliser is applied.
  nsub,                                                                       &
...
REAL ::                                                                       &
  ...
  n_fix_denom,                                                                &
    ! Denominator for fixation calc l_layeredC=T.
  disturb_crop
...
REAL ::                                                                       &
  ...
  n_unmet(land_pts,npft),                                                     &
    ! N this unmet through exudate limit (kg m-2 [360days]-1).
  n_unmet_gb(land_pts),                                                       &
    ! N this unmet through exudate limit (kg m-2 [360days]-1).
  deposition(land_pts),                                                       &
    ! N deposition (kgN m-2 360days-1).
  n_avail(land_pts,npft),                                                     &
    ! Inorganic N on tiles (kgN m-2).
  neg_n(land_pts),                                                            &
...
IF (l_layeredC) THEN
  ! Calculate fraction of roots in each soil layer, and which layers unfrozen.
  DO n = 1,npft
    CALL root_frac(n, dim_cslayer, dzsoil, rootd_ft(n), f_root_pft(n,:))
  END DO
  ...
  DO t = 1,trif_pts
    l = trif_index(t)
    DO nn = 1,dim_cslayer
      IF ( t_soil_soilt_acc(l,1,nn) < zerodegc ) isunfrozen(l,nn) = 0.0
    END DO
  END DO
  ! Also (to be used later) calculate the layer closest to 30cm depth.
  zlyr = 0
  DO nn = dim_cslayer,1,-1
    IF (SUM(dzsoil(1:nn)) >= 0.3) zlyr = nn
  END DO
  IF (zlyr == 0) zlyr = dim_cslayer !if soil depth < 30cm, fertilizer in all
END IF ! end if layeredc
...
!-----------------------------------------------------------------------------
! Nitrogen deposition and fixing.
!-----------------------------------------------------------------------------
DO t = 1,trif_pts
  l = trif_index(t)

  ! Convert deposition from kgN/m2/s to kgN/m2/360days.
  ! Note that deposition_N is the instantaneous rate. Any variation on
  ! timescales shorter than triffid_period will be lost.
  deposition(l) = MAX(deposition_N_gb(l) * 60.0 * 60.0 * 24.0 * 360.0, 0.0)

  !---------------------------------------------------------------------------
  ! Make deposition available for vegetation uptake, top layer only
  !---------------------------------------------------------------------------
  n_inorg_soilt_lyrs(l,1,1) = n_inorg_soilt_lyrs(l,1,1) + deposition(l) /     &
                              r_gamma
  IF (l_layeredC) THEN
    DO n = 1,npft
      n_inorg_avail_pft(l,n,1) = n_inorg_avail_pft(l,n,1) +                   &
                                 (deposition(l) * f_root_pft(n,1) / r_gamma)
    END DO
  END IF

  ! Calculate fixing.
  DO n = 1,npft
    n_fix_pft(l,n) = MAX(npp(l,n) * 1.6 / 1.0e3, 0.0)
                    ! Fixation rate 1.6 gN(kg C)-1
    N_fix_gb(l) = N_fix_gb(l) + frac(l,n) * n_fix_pft(l,n)
  END DO

  !---------------------------------------------------------------------------
  ! Make fixation available for vegetation uptake
  ! If layered nitrogen fixation is distributed according to root profile
  !---------------------------------------------------------------------------
  IF (l_layeredC) THEN
    IF (SUM(isunfrozen(l,:)) > 0.0) THEN !depends on frozen status
      DO n = 1,npft
        n_fix_denom = 1 / (r_gamma * SUM(isunfrozen(l,:)  * f_root_pft(n,:)))
        DO nn = 1,dim_cslayer
          n_inorg_soilt_lyrs(l,1,nn) = n_inorg_soilt_lyrs(l,1,nn)             &
            +  (n_fix_pft(l,n) * frac(l,n) * f_root_pft(n,nn)                 &
                * isunfrozen(l,nn)) * n_fix_denom
          n_inorg_avail_pft(l,n,nn) = n_inorg_avail_pft(l,n,nn)               &
            +  (n_fix_pft(l,n) * f_root_pft(n,nn)                             &
                * isunfrozen(l,nn)) * n_fix_denom
        END DO
      END DO
    ELSE !if whole soil is frozen, just put it in the top layer
      n_inorg_soilt_lyrs(l,1,1) = n_inorg_soilt_lyrs(l,1,1) +                 &
                                  n_fix_gb(l) / r_gamma
      DO n = 1,npft
        n_inorg_avail_pft(l,n,1) = n_inorg_avail_pft(l,n,1)                   &
                                   + n_fix_pft(l,n) / r_gamma
      END DO
    END IF
...
DO n = 1,npft
  ...
  !---------------------------------------------------------------------------
  IF (l_layeredC) THEN
    n_avail(:,n) = n_inorg_avail_pft(:,n,1) * isunfrozen(:,1)
    DO nn = 2,dim_cslayer
      n_avail(:,n) = n_avail(:,n) +                                           &
...
IF ( l_veg_compete ) THEN
  ...
  IF ( l_trif_crop ) THEN
    ...
    !set fractions without fire equal to fractions with fire
    !because fire is currently incompatable with l_trif_crop
    frac_nofire(:,:)  = frac(:,:)
    dfrac_nofire(:,:) = dfrac(:,:)
  ELSE
    ...
    IF ( l_ht_compete ) THEN
      ...
      ELSE
        ...
        CALL lotka_noeq( land_pts, trif_pts, trif_index,                      &
                         ...
                         ht, pc_s_pft,                                        &
                         frac, frac_na,                                       &
                         frac_nofire, dfrac, dfrac_na, dfrac_nofire )
...
DO t = 1,trif_pts
  ...
  !---------------------------------------------------------------------------
  ! Add FERTILIZER
  ! If layered spread it in the top 30cm (usual ploughing depth)
  !---------------------------------------------------------------------------

  IF (l_layeredC) THEN
    n_inorg_soilt_lyrs(l,1,1:zlyr) = n_inorg_soilt_lyrs(l,1,1:zlyr) +         &
                                     (n_fertiliser_gb(l) * dzsoil(1:zlyr) /   &
                                     (SUM(dzsoil(1:zlyr)) * r_gamma))
    DO n = 1,npft
      n_inorg_avail_pft(l,n,1:zlyr) = n_inorg_avail_pft(l,n,1:zlyr) +         &
                                      (n_fertiliser_pft(l,n) * dzsoil(1:zlyr) &
                                      * f_root_pft(n,1:zlyr)) /               &
                                      (SUM(dzsoil(1:zlyr)) * r_gamma)
    END DO
  ELSE !IF .NOT. l_layeredC
    n_inorg_soilt_lyrs(l,1,1) = n_inorg_soilt_lyrs(l,1,1) +                   &
                                n_fertiliser_gb(l) / r_gamma
  END IF
  ...
  DO n = 1,npft
    ...
    !-------------------------------------------------------------------------
    IF (l_layeredC) THEN
      !not a gb value for l_layeredC
      n_inorg_gb(l) = SUM(n_inorg_avail_pft(l,n,:) * isunfrozen(l,:))
    ELSE
      n_inorg_gb(l) = n_inorg_soilt_lyrs(l,1,1)
    END IF

    IF (l_nitrogen .AND. n_uptake_pft(l,n) / r_gamma > n_inorg_gb(l)) THEN
      n_unmet(l,n)      = n_uptake_pft(l,n) - (n_inorg_gb(l) * r_gamma)
    ...
    IF (l_layeredC) THEN
      IF (n_inorg_gb(l) > 0.0) THEN
        n_inorg_soilt_lyrs(l,1,:) = n_inorg_soilt_lyrs(l,1,:) -               &
                                    (n_uptake_pft(l,n) * frac_flux(l,n) *     &
                                    n_inorg_avail_pft(l,n,:) *                &
                                    isunfrozen(l,:)) / (r_gamma *             &
                                    n_inorg_gb(l))
        n_inorg_avail_pft(l,n,:)  = n_inorg_avail_pft(l,n,:) -                &
                                    (n_uptake_pft(l,n) *                      &
                                     n_inorg_avail_pft(l,n,:) *               &
                                     isunfrozen(l,:)) /                       &
      ...
      ELSE IF (n_inorg_gb(l) == 0.0) THEN
        IF (n_uptake_pft(l,n) * frac_flux(l,n) < 0.0) THEN
          n_inorg_soilt_lyrs(l,1,:) = n_inorg_soilt_lyrs(l,1,:) -             &
                                      (n_uptake_pft(l,n) * frac_flux(l,n) *   &
                                       f_root_pft(n,:)) / r_gamma
          n_inorg_avail_pft(l,n,:)  = n_inorg_avail_pft(l,n,:) -              &
                                      (n_uptake_pft(l,n) *                    &
                                       f_root_pft(n,:)) / r_gamma
        ELSE IF (n_uptake_pft(l,n) * frac_flux(l,n) > 0.0) THEN
          n_unmet(l,n)      = n_unmet(l,n) + n_uptake_pft(l,n)
    ...
    n_unmet_gb(l)  = n_unmet_gb(l)  + frac_flux(l,n) * n_unmet(l,n)
...
IF ( soil_bgc_model == soil_model_rothc ) THEN
  ...
  DO t = 1,trif_pts
    ...
    IF (l_layeredC) THEN !turnover on soil levels using tau_resp
      DO nn = 1,dim_cslayer
        inorg_decay = EXP( -tau_resp * (SUM(dzsoil(1:nn)) -                   &
                           0.5 * dzsoil(nn)) ) * isunfrozen(l,nn)
        ...
        DO n = 1,npft
          ...
          IF (l_trif_eq .OR. (diff_n_pft > r_gamma * 0.5) ) THEN
            n_inorg_avail_pft(l,n,nn) = f_root_pft(n,nn) *                    &
                                        n_inorg_soilt_lyrs(l,1,nn)
          ELSE
            n_inorg_avail_pft(l,n,nn) = n_inorg_avail_pft(l,n,nn) +           &
                                        (diff_n_pft * ( (f_root_pft(n,nn) *   &
                                        n_inorg_soilt_lyrs(l,1,nn)) -         &
                                        n_inorg_avail_pft(l,n,nn) ) / r_gamma)
          END IF
        END DO
