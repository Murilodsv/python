...
! Local variables
INTEGER :: spt, i, j, l, n ! do loop variables
...
DO spt = 1,soil_pts
  ...
  IF (l_new_qsat_jules) THEN
    CALL qsat_new(sat_spec_humidity, t1p5m(i,j), pstar(i,j))
  ELSE
    ! DEPENDS ON: qsat
    CALL qsat(sat_spec_humidity, t1p5m(i,j), pstar(i,j), 1)
  END IF
...
! Subroutine arguments
REAL, INTENT(IN) :: temperature_degC
