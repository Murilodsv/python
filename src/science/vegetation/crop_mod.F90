...
REAL :: nonyield_diag_dummy = 0.0  ! Dummy variable used when making
     ! sure that crop is consistent at initialisation if it's not sown.

REAL :: dvi_dummy = 0.0  ! Dummy variable used to make sure dvi is not
                         ! overwritten when initialising crop at beginning
                         ! of run.

REAL :: dvi_prev1 ! variable to store previous value of DVI
REAL :: dvi_prev2 ! variable to store previous value of DVI
  ...
  DO j = 1,surft_pts(nnpft + n)
    ...
    IF ( dvi(l,n) >= initial_c_dvi(n) ) THEN
      ...
      ELSE IF ( l_prescsow ) THEN
        ...
        END IF
        IF (day_of_yr == day_before_sowdate .OR. day_of_yr == latestharv_date_cpft(l,n)) THEN
          harvest_counter_cpft(l,n) = 1
