...
REAL, DIMENSION(12), PARAMETER  :: day_length  =                              &
  (/ 6.5, 7.5, 9.0, 12.8, 13.9, 13.9, 12.4, 10.9, 9.4, 8.0, 7.0, 6.0 /)
REAL, DIMENSION(12), PARAMETER  :: day_len_adj =                              &
  (/ -1.6, -1.6, -1.6, 0.9, 3.8, 5.8, 6.4, 5.0, 2.4, 0.4, -1.6, -1.6 /)

REAL, PARAMETER                 :: canopy_rain_loss = 0.5,                    &
...
REAL,     INTENT(INOUT) :: ffmc(:),        & !fine fuel moisture Code
                           mois_ffmc(:),   & !moisture part of ffmc
                           dmc(:),         & !Duff Moisture Code
                           dc(:)             !Drought Code

! < Array arguments with INTENT(OUT)>
REAL,     INTENT(OUT)   :: isi(:),         & !Initial spread Index
                           bui(:),         & !Buildup Index
                           fwi(:)            !Fire Weather Index
...
! ! Subroutine arguments
! < Array arguments with INTENT(IN) >
REAL, INTENT(IN)      :: rel_hum(:),                                          &
                         temperature(:),                                      &
                         wind(:),                                             &
                         rain(:)

! < Array arguments with INTENT(INOUT)>
REAL, INTENT(INOUT)   :: mois_ffmc(:)
...
! < Array arguments with INTENT(INOUT)>
REAL,     INTENT(INOUT) :: dmc(:)

! < Scalar local variables >
INTEGER :: alt_iMonth

! < Array local variables >
REAL    :: k(SIZE(rain)),             & !log drying rate,log10M/day
...
! < Array arguments with INTENT(INOUT)>
REAL,     INTENT(INOUT) :: dc(:)

! < Scalar local variables >
INTEGER :: alt_iMonth

! < Array local variables >
REAL    :: v(SIZE(rain)),                  & !pot evapotranspiration,
                                             !(0.254 mm water/day)
...
!
! < Array arguments with INTENT(IN) >
REAL,   INTENT(IN), DIMENSION(:)          :: dmc
REAL,   INTENT(IN), DIMENSION(:)          :: dc

! < Array arguments with INTENT(OUT)>
REAL,   INTENT(OUT), DIMENSION(:)         :: bui
