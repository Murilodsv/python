...
REAL, PARAMETER   :: inv_t_ref      = 1.0 / 29.5858
REAL, PARAMETER   :: inv_h_ref      = 1.0 / 28.9855
REAL, PARAMETER   :: inv_w_ref      = 1.0 / 42.735

!Threshold for a 'wet' day. Set to a nominal low value.
REAL, PARAMETER   :: precip_thresh  = 0.01
...
REAL ,   INTENT(IN)     ::  temp(:),        & !Daily max Temperature (C)
...
REAL,    INTENT(INOUT)  ::  r_drought(:),   & !24h accum. on last rain day
                            n_drought_p1(:)   !N days since last rain+1

! < Array arguments with INTENT(OUT)>
REAL,    INTENT(OUT)    ::  ffdi(:)

! < Array local variables >
REAL                    ::  drought_term(SIZE(temp)),                         &
