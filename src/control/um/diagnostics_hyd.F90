...
USE prognostics, ONLY: snowdepth_surft
...
USE fire_vars, ONLY: burnt_area, burnt_area_ft, emitted_carbon,               &
    emitted_carbon_ft, emitted_carbon_DPM, emitted_carbon_RPM,                &
    fire_em_CO2, fire_em_CO, fire_em_CH4, fire_em_NOx,                        &
    fire_em_CO2_ft, fire_em_CO_ft, fire_em_CH4_ft, fire_em_NOx_ft,            &
    fire_em_SO2, fire_em_OC, fire_em_BC,                                      &
    fire_em_SO2_ft, fire_em_BC_ft, fire_em_OC_ft
...
IF (sf(376,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
     " : error in set_pseudo_list(item 376 = hsnow_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),snowdepth_surft(:,pslevel_out),                 &
  ...
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),tsurf_elev_surft(:,pslevel_out),                &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
  ...
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),snow_soil_htf(:,pslevel_out),                   &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
...
IF (sf(578,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
      " : error in set_pseudo_list(item 578 = snice_smb_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
...
IF (sf(579,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
        " : error in set_pseudo_list(item 579 = snice_m_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),sf_diag%snice_m_surft(:,pslevel_out),           &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
...
IF (sf(580,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
     " : error in set_pseudo_list(item 580 = snice_freez_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
...
IF (sf(581,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
  " : error in set_pseudo_list(item 581 = snice_sicerate_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
...
IF (sf(582,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
  " : error in set_pseudo_list(item 582 = snice_sliqrate_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
...
IF (sf(583,8)) THEN
  ...
  IF (icode >  0) THEN
    cmessage= RoutineName //                                                  &
    " : error in set_pseudo_list(item 583 = snice_runoff_surft)"
    GO TO 9999
  END IF
  pslevel_out = 0
  DO pslevel = 1,nsurft
...
IF (sf(202,8)) THEN
  snomlt_surf_htf_1d = RESHAPE(snomlt_surf_htf, (/row_length * rows/))
  CALL expand_from_mask (                                                     &
  ...
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),snow_surft(:,pslevel_out),                      &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
  ...
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),melt_surft(:,pslevel_out),                      &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
  ...
  DO pslevel = 1,nsurft
    IF (plltile(pslevel)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),rgrain(:,pslevel_out),                          &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
  ...
  DO level = 1,dsm_levels
    IF (list(level)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),sthu(:,level_out),                              &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
  ...
  DO level = 1,dsm_levels
    IF (list(level)) THEN
      ...
      CALL expand_from_mask (                                                 &
          ...
          *row_length * rows),sthf(:,level_out),                              &
          land_sea_mask,row_length * rows,land_points)
    END IF
  END DO
END IF
...
IF ( l_inferno ) THEN
  ...
  IF ( sf(700, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = burnt_area(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag( stashwork(si(700, 8, im_index)), interp_data,              &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 700,                                                    &
         icode, cmessage )

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 700)"
      GO TO 9999
    END IF
  ...
  IF ( sf(701, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = emitted_carbon(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(701, 8, im_index)), interp_data,               &
         row_length,rows, 0, 0, 0, 0, at_extremity,                           &
         atmos_im, 8, 701,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 701)"
      GO TO 9999
    END IF
  ...
  IF ( sf(702, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = emitted_carbon_dpm(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(702, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 702,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 702)"
      GO TO 9999
    END IF
  ...
  IF ( sf(703, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = emitted_carbon_rpm(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(703, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 703,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 703)"
      GO TO 9999
    END IF
  ...
  IF ( sf(710, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 710 = burnt_area_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = burnt_area_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(710, 8, im_index) +                        &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8710)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(711, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 711 = emitted_carbon_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = emitted_carbon_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(711, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 711,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8711)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(720, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_co2(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(720, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 720,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 720)"
      GO TO 9999
    END IF
  ...
  IF ( sf(721, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_co(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(721, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 721,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 721)"
      GO TO 9999
    END IF
  ...
  IF ( sf(722, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_ch4(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(722, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 722,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 722)"
      GO TO 9999
    END IF
  ...
  IF ( sf(723, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_nox(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(723, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 723,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 723)"
      GO TO 9999
    END IF
  ...
  IF ( sf(735, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 735 = fire_em_co2_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_co2_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(735, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 735,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8735)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(736, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 736 = fire_em_co_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_co_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(736, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 736,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8736)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(737, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 737 = fire_em_ch4_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_ch4_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(737, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 737,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8737)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(738, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 738 = fire_em_nox_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_nox_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(738, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 738,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8738)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(750, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_so2(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(750, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 750,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 750)"
      GO TO 9999
    END IF
  ...
  IF ( sf(751, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_bc(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(751, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 751,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 751)"
      GO TO 9999
    END IF
  ...
  IF ( sf(752, 8) ) THEN
    ...
    DO l = 1, land_points
      j = (land_index(l) - 1) / row_length + 1
      i = land_index(l) - (j-1) * row_length
      interp_data(i, j) = fire_em_oc(l)
    END DO

    ! DEPENDS ON: copydiag
    CALL copydiag(stashwork(si(752, 8, im_index)), interp_data,               &
         row_length, rows, 0, 0, 0, 0, at_extremity,                          &
         atmos_im, 8, 752,                                                    &
         icode, cmessage)

    IF ( icode  > 0 ) THEN
      cmessage = "Error in copydiag(item 752)"
      GO TO 9999
    END IF
  ...
  IF ( sf(765, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 765 = fire_em_so2_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_so2_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(765, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 765,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8765)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(766, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 766 = fire_em_bc_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_bc_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(766, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 766,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8766)"
          GO TO 9999
        END IF
      END IF
    END DO
  ...
  IF ( sf(767, 8) ) THEN
    ...
    IF ( icode > 0 ) THEN
      cmessage=                                                               &
      "diag_hyd : error in set_pseudo_list (item 767 = fire_em_oc_ft)"
      GO TO 9999
    END IF
    pslevel_out = 0
    ...
    DO pslevel = 1, npft
      IF (pllpft(pslevel)) THEN
        ...
        DO l = 1, land_points
          j = (land_index(l) - 1) / row_length + 1
          i = land_index(l) - (j-1) * row_length
          interp_data(i, j) = fire_em_oc_ft(l, pslevel_out)
        END DO

        ! DEPENDS ON: copydiag
        CALL copydiag(STASHwork(si(767, 8, im_index) +                        &
                      (pslevel_out-1) * row_length * rows),                   &
                      interp_data,                                            &
                      row_length,rows, 0, 0, 0, 0, at_extremity,              &
                      atmos_im, 8, 767,                                       &
        ...
        IF (icode > 0) THEN
          cmessage="Error in copydiag (item 8767)"
          GO TO 9999
        END IF
      END IF
    END DO
