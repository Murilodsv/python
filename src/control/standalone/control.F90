...
!Imports for the remaining calculations in control
...
!Explicit
USE prognostics,            ONLY:                                             &
  canopy_surft, snow_surft, k_sice_sicat, cs_pool_soilt, canht_pft, lai_pft,  &
  t_soil_soilt, tsurf_elev_surft, ti_sicat, tstar_surft,                      &
  z0msea_ij, gs_gb, smc_soilt, gc_surft
USE ancil_info,             ONLY:                                             &
  land_pts, npft_trif,dim_cs1, dim_cs2, co2_dim_len,co2_dim_row,              &
  z1_uv_ij, z1_tq_ij, land_index, nsurft, ice_fract_ncat_sicat,               &
  ti_cat_sicat, frac_surft, surft_index, surft_pts, land_pts_trif
USE coastal,                ONLY:                                             &
...
USE trifctl,                ONLY:                                             &
  ...
  resp_s_acc_soilt, gpp_gb, npp_gb, resp_p_gb, g_leaf_pft, gpp_pft, npp_pft,  &
  resp_p_pft, resp_s_soilt, resp_w_pft
USE p_s_parms,              ONLY:                                             &
  catch_surft, catch_snow_surft, hcon_soilt, smvccl_soilt, smvcst_soilt,      &
  smvcwt_soilt, sthf_soilt, sthu_soilt, z0_surft, z0h_bare_surft,             &
  z0m_soil_gb, albsoil_soilt, cosz_ij, clay_soilt
USE orog,                   ONLY:                                             &
...
! Local variables. In the vast majority of cases, these are arguments only
! required when coupled to the UM
REAL ::                                                                       &
